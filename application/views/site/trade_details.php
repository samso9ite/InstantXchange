	...
	<div class="row">
		<div class="col-lg-12" id="faq">
			...
			<?php if(!$trade): ?>
				...
				<?php else: ?>
					<div class="col-sm-12" style="padding: 15px;margin-bottom: 30px; border: 1px solid #e1e1e1;">
						<div class="col-sm-6">
							<br>
								Trade Type: <?=ucfirst($trade->type)?>
						</div>
						<div class="col-sm-6">
							<br>
							<h6>Total Amount: &#8358;<?=number_format($trade->amount_naira,2)?></h6>
						</div>	
						<?php if($trade->bank): ?>
						<div class="col-sm-6">
						<br>
							<h6>Bank: <?=$trade->bank?> </h6>
						</div>
						<div class="col-sm-6">
						<br>
							<h6>Account Number: <?=$trade->account_number?></h6>
						</div>
						<div class="col-sm-6">
						<br>
							<h6>Account Name: <?=$trade->account_name?></h6>
						</div>
						<?php else: ?>
							<div class="col-sm-6">
							<br>
							...
							<div class="col-sm-12" style="padding: 0px;">
								<table class="table table-bordered table-responsive">
									...
									<?php foreach($details as $row): ?>
										<tr>
											<td><?=ucfirst($row->giftcardtype)." $".$row->usd?></td>
											<td>&#8358;<?=$row->naira?></td>
											<td><?=$row->quantity?></td>
											<td>&#8358;<?=number_format($row->quantity * $row->naira,2)?></td>
										</tr>
									<?php endforeach; ?>
								</table>
							</div>
									...
									<?php foreach($images as $img): ?>
										<div class="col-sm-3" style="margin-bottom: 10px;">
											<a class="img" href="<?=base_url('uploads/'.$img->imagename)?>">
												<img src="<?=base_url('uploads/'.$img->imagename)?>" class="img-fluid">
											</a>
										</div>
									<?php endforeach; ?>
								</div>
								...
								<div class="row">
									<?php if(!empty($proof)): ?>
										<?php foreach($proof as $img): ?>
											<div class="col-sm-3">
												<a class="img" href="<?=base_url('uploads/'.$img->image)?>">
													<img src="<?=base_url('uploads/'.$img->image)?>" class="img-fluid" width="500">
												</a>
											</div>
										<?php endforeach; ?>
										<?php else: ?>
						...
						<?php if($this->session->userdata('admin') != "" && $trade->status == "Pending"): ?>
							<div class="main_hold">
								<a href="<?=site_url('admin/close_trade/'.$trade->ref_code)?>" class="complete" 
								...
								<div class="modal fade" id="comment">
									<div class="modal-dialog modal-lg">
										<div class="modal-content">
											...
											<form method="post" action="<?=site_url('admin/trade_cancel')?>" enctype="multipart/form-data" >
												<div class="row">
													<div class="col-lg-12">
